...
async function testFinalFix() {
    try {
        console.log('🚀 Test cuối cùng với code đã sửa input.json');
        ...
        const campaignData = {
            name: 'Test Final Fix - Input.json',
            description: 'Test actor với input.json được ghi đúng vị trí',
            actorId: actor._id,
            input: simpleInput
        };
        ...
        while (status === 'running' && attempts < maxAttempts) {
            ...
            try {
                ...
                if (status === 'completed') {
                    ...
                    console.log(`Log: ${campaignStatus.result.log}`);
                    break;
                }
            } catch (error) {
