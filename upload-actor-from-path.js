...
async function uploadActorFromPath() {
    try {
        console.log('🚀 Uploading actor from D:\\actor-craw-by-class');
        ...
        // 3. Tạo actor mới trong database
        console.log('\n2️⃣ Creating new actor in database...');
        const actorData = {
            name: 'Actor Craw by Class (Latest)',
            description: 'Actor crawl sản phẩm từ website với input schema tùy chỉnh - phiên bản mới nhất',
            type: 'web-scraper',
            inputSchema: {
                type: 'object',
                properties: {
        ...
        // 5. Cập nhật path trong database
        ...
        // Kiểm tra file hung.json trong src
        const hungJsonSrcPath = path.join(targetDir, 'src', 'hung.json');
        if (fs.existsSync(hungJsonSrcPath)) {
            const stats = fs.statSync(hungJsonSrcPath);
            console.log(`📋 Found src/hung.json: ${(stats.size / 1024).toFixed(2)} KB`);
        }
